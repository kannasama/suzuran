...
| `src/jobs/mod.rs` | `JobHandler` trait + `ScanPayload` + `OrganizePayload` + `FingerprintPayload` + `CueSplitPayload` + `TranscodePayload` + `ArtProcessPayload` + `NormalizePayload` DTOs |
