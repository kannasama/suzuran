...
| `src/api/mod.rs` | `api_router()` — mounts `/auth`, `/totp`, `/webauthn`, `/settings`, `/themes`, `/libraries`, `/jobs`, `/tracks`, `/organization-rules`, `/tag-suggestions`, `/encoding-profiles`, `/art-profiles`, `/virtual-libraries`, `/uploads` subrouters; `.merge(transcode::router())` and `.merge(art::router())` for direct-path routes |
