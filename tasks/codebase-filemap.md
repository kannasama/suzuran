...
| `ui/src/api/` | `client.ts` (Axios baseURL `/api/v1`), `auth.ts`, `libraries.ts` (Library type includes `encoding_profile_id: number \| null`), `organizationRules.ts`, `tagSuggestions.ts`, `tracks.ts`, `encodingProfiles.ts` (list, create, update, delete → `/encoding-profiles`), `artProfiles.ts` (list, create, update, delete → `/art-profiles`), `transcode.ts` (`transcodeApi` — transcodeTrack, transcodeLibrary, transcodeSync), `art.ts` (`artApi` — embedFromUrl, extract, standardize, standardizeLibrary) |
