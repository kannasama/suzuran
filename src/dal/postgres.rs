...
impl Store for PgStore {
    ...
    async fn list_tracks_by_profile(&self, library_id: i64, library_profile_id: Option<i64>) -> Result<Vec<Track>, AppError> {
        sqlx::query_as::<_, Track>(
            "SELECT * FROM tracks WHERE library_id = $1 AND library_profile_id IS NOT DISTINCT FROM $2 ORDER BY id",
        )
        .bind(library_id)
        ...
        sqlx::query_as::<_, Track>(
            "INSERT INTO tracks (library_id, relative_path, file_hash, title, artist, albumartist,
             ...
             ON CONFLICT (library_id, relative_path) DO UPDATE SET
               ...
               status=$24, library_profile_id=$25, last_scanned_at=NOW()
             RETURNING *",
        )
    ...
    async fn enqueue_job_after(
        ...
        .bind(job_type).bind(payload).bind(priority).bind(run_after)
        .fetch_one(&self.pool).await.map_err(AppError::Database)
    }
