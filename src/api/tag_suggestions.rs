...
async fn accept(
    ...
    let suggestion = state.db.get_tag_suggestion(id).await?
        .ok_or_else(|| AppError::NotFound(format!("tag_suggestion {id}")))?;
    let (fields, apply_art) = body
        .map(|b| (b.0.fields, b.0.apply_art))
        .unwrap_or((None, true));
    crate::services::tagging::apply_suggestion(&state.db, &suggestion, fields.as_deref(), apply_art).await?;
...
async fn batch_accept(
    ...
    for s in suggestions.iter().filter(|s| s.confidence >= body.min_confidence) {
        crate::services::tagging::apply_suggestion(&state.db, s, None, true).await?;
